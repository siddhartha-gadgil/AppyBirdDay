...
var size= querySelector('#size_input');
...
void repUpdate(){
  querySelector("#rep").text = rep.toString();
}
...
StreamSubscription sizeQuery(Map<String, Map<String, String>> features){
  ...
  querySelector("#size").children = [sizeElem];
  return subsc;
}
...
void main() {
  ...
  var sizeclick = querySelector('#size_input').onChange.listen((Event e){
    setSize(size.value, rep);});  

  sizeQuery(rep);
  

  querySelector('#colour_list').children = colourQuery(mainColours, rep);
...
var wikiSample = JSON.decode(wikiSampleText);
